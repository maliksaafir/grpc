...
int DefaultSslRootStore::FetchPEMRoots_MountainLion(CFDataRef *pemRoots) {
	if (pemRoots == NULL) {
		return -1;
	}
...
int DefaultSslRootStore::useOldCode() {
	char str[256];
...
int DefaultSslRootStore::FetchPEMRoots(CFDataRef *pemRoots, CFDataRef *untrustedPemRoots) {
	if (useOldCode()) {
